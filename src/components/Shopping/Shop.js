...
function parseItems(arrayOfitems) {
    ...
    arrayOfitems.forEach(function(eachItem) {
        ...
        if(eachItem.children) {
            ...
            return collapsibleArray.push(<Item precaution={eachItem.precaution} howtouse = {eachItem.howtouse} price = {eachItem.price} title={eachItem.title} type={eachItem.type} imagePath={eachItem.imagePath} desc={eachItem.desc} goodFor={eachItem.goodFor} quantity={eachItem.quantity} flavours={eachItem.flavours} ingredients={eachItem.ingredients}></Item>);
